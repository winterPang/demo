;(function($)
{
	...
	function portletForm(jPortLet, isToggle)
	{
		...
		function init (jPortLet)
		{
            ...
            _jTools = $(".form-actions", jContainer).empty();
            if(_jTools.length == 0)
            {
            	_jTools = $('<div class="form-actions hide"></div>').appendTo (jContainer);
            }
	...
	function formHead (jForm, opt)
	{
		...
		function appendTools (oTools)
		{
			function makeButton(oButton, jForm,sType)
			{
				function box()
				{
					jButton = $('<a href="javascript:void(0)" class="form-btn '+oButton.icon+'" title="'+ oButton.text +'"></a>');
	...
	var oForm = {
		...
		updateForm: function(oData, para)
		{
			...
			$("input:[type='radio'], input:[type='checkbox'], select", jForm).off("change.form");
			var option = {prefixId: ""};
        	...
        	$("input:[type='radio'], input:[type='checkbox'],select", jForm).on("change.form",function(){
